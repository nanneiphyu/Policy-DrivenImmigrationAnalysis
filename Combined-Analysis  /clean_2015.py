...
# Filtering and Compiling data for 2015
# Lawful Permanent Residents 


lpr = pd.read_csv('fy2015_immsuptable2d.csv', skiprows=2, header=1)
## Replacing missing values with NaN
pd.set_option('future.no_silent_downcasting', True)
lpr_2015 = lpr.replace(['D', '-'], np.nan)
lpr_2015 = lpr_2015.dropna(axis=1, how='all')
print(lpr_2015)


## Selecting Specific rows for the dataframe 
lpr_2015 = lpr_2015.iloc[2:9]
## Checking the result 
print(lpr_2015.columns)
## Prepare for the barchart 

lpr_2015 = lpr_2015.reset_index(drop=True)
lpr_2015 = lpr_2015.set_index("Region and country of birth")
lpr_2015 = lpr_2015.rename_axis('Region')
print(lpr_2015)
##
lpr_2015.to_csv('lpr_2015_region.csv')


## Just with two columns 
lpr_2015_region = pd.read_csv('lpr_2015_region.csv', thousands=',')

## Sorting values 
lpr_2015_region = lpr_2015_region.sort_values('Total', ascending=True).reset_index(drop=True)
print(lpr_2015_region)
##

#Select rows to keep 
lpr_2015_region = lpr_2015_region [['Region', 'Total']]
...
bars = ax1.barh(lpr_2015_region['Region'], 
                lpr_2015_region['Total'], 
                color='navy')
max_value = lpr_2015_region ['Total'].max()
## Annotating 
for index, value in enumerate(lpr_2015_region['Total']):
    if value >= 1000:  
        ax1.text(value, index, shortened_number(value), va='center', ha='left', fontsize=11, rotation=360) 
    
ax1.set_title('Total Number of Lawful Permanent Residents by Region (2015)')
ax1.set_xlabel('Numbers of LPRs')
ax1.set_ylabel('Region')
ax1.set_xlim(0, max_value * 1.10)
fig.tight_layout()
fig.savefig('lpr_2015_region.png', dpi=300)
...
lpr_2015_country = pd.read_csv('fy2015_immsuptable2d.csv', skiprows=2, header=1)
pd.set_option('future.no_silent_downcasting', True)
## Replacing missing values with NaN
lpr_2015_country = lpr_2015_country.replace(['D', '-'], np.nan)
# check
print(lpr_2015_country)
print(lpr_2015_country.columns.tolist())
print(lpr_2015_country.head())

## Selecting Specific rows for the dataframe 
lpr_2015_country = lpr_2015_country.iloc[11:-5]
print(lpr_2015_country)

lpr_2015_filtered = lpr_2015_country.rename(columns={"Region and country of birth": "Country"})
## Remove columns that are not necessary 
lpr_2015_filtered = lpr_2015_filtered.drop(columns=['Non-CBSA', 'Other & Unknown'])
## drop missing data 
lpr_cols = lpr_2015_filtered.columns.drop('Country')
lpr_2015_final = lpr_2015_filtered.dropna(subset=lpr_cols, how='all').reset_index(drop=True)
## Check again 
print(lpr_2015_final)
## Saving files for the map - that will be need later
lpr_2015_final.to_csv('lpr_2015_by_country.csv', index=False)
...
lpr_2015_country_total = pd.read_csv('lpr_2015_by_country.csv')
...
lpr_countries = set(lpr_2015_country_total['Country'].str.strip())
...
lpr_2015_country_total['Country'] = lpr_2015_country_total['Country'].replace(to_rename_csv)
#
# Dropping names that are not relevant 
to_drop_csv = [
    'Soviet Union (former)',
...
countries_dropped = lpr_2015_country_total[~lpr_2015_country_total['Country'].isin(to_drop_csv)].reset_index(drop=True)
...
merged_iso_csv.to_csv('lpr_added_iso_cbsa_2015.csv', index=False)
...
merged_iso_csv_final.to_csv('lpr_2015_country_total_only.csv', index=False)
## Saved this for country specific maps 

#%%
## Top 20 countries of lpr flow in 2015

# Reading file
merged_lpr = pd.read_csv('lpr_2015_country_total_only.csv', thousands=',')
...
ax.set_title('Top 20 Countries by Total LPRs (2015)')
...
fig.tight_layout()
fig.savefig('top_20_lpr_2015.png', dpi=300)
# Saved the figure for top lpr by country of birth 

#%%
# Let's focus on the data with csba areas
cbsa_2015 = pd.read_csv('lpr_added_iso_cbsa_2015.csv', thousands=',')
cbsa_2015 = cbsa_2015.drop(columns=['Total', 'NAME_LONG', 'ISO_A3'])
cbsa_transposed = cbsa_2015.set_index('Country').T
...
#     
merged_join_key.to_csv('merged_join_key_2015_for_QGIS.csv', index=False)

# Saved the file 
## Join in QGIS to show it on the map and 2015 analysis complete
###### COMPLETE for 2015
