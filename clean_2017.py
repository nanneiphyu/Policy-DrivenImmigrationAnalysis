...
# Filtering and Compiling data for 2017
# Lawful Permanent Residents 


lpr = pd.read_csv('fy2017_immsuptable2d.csv', skiprows=2, header=1)
## Replacing missing values with NaN
pd.set_option('future.no_silent_downcasting', True)
lpr_2017 = lpr.replace(['D', '-'], np.nan)
lpr_2017 = lpr_2017.dropna(axis=1, how='all')
print(lpr_2017)


## Selecting Specific rows for the dataframe 
lpr_2017 = lpr_2017.iloc[2:9]
## Checking the result 
print(lpr_2017.columns)
## Prepare for the barchart 

lpr_2017 = lpr_2017.reset_index(drop=True)
lpr_2017 = lpr_2017.set_index("Region and country of birth")
lpr_2017 = lpr_2017.rename_axis('Region')
print(lpr_2017)
##
lpr_2017.to_csv('lpr_2017_region.csv')



## Just with two columns 
lpr_2017_region = pd.read_csv('lpr_2017_region.csv', thousands=',')

## Sorting values 
lpr_2017_region = lpr_2017_region.sort_values('Total', ascending=True).reset_index(drop=True)
print(lpr_2017_region)
##

#Select rows to keep 
lpr_2017_region = lpr_2017_region [['Region', 'Total']]
...
bars = ax1.barh(lpr_2017_region['Region'], 
                lpr_2017_region['Total'], 
                color='teal')
max_value = lpr_2017_region ['Total'].max()

for index, value in enumerate(lpr_2017_region['Total']):
    if value >= 1000:  
        ax1.text(value, index, shortened_number(value), va='center', ha='left', fontsize=11, rotation=360) 
    
ax1.set_title('Total Number of Lawful Permanent Residents by Region (2017)')
ax1.set_xlabel('Numbers of LPRs')
ax1.set_ylabel('Region')
ax1.set_xlim(0, max_value * 1.10)
fig.tight_layout()
fig.savefig('lpr_2017_region.png', dpi=300)
...
lpr_2017_country = pd.read_csv('fy2017_immsuptable2d.csv', skiprows=2, header=1)
pd.set_option('future.no_silent_downcasting', True)
## Replacing missing values with NaN
lpr_2017_country = lpr_2017_country.replace(['D', '-'], np.nan)
# check
print(lpr_2017_country)
print(lpr_2017_country.columns.tolist())
print(lpr_2017_country.head())

## Selecting Specific rows for the dataframe and check again
lpr_2017_country = lpr_2017_country.iloc[11:-10]
print(lpr_2017_country)

lpr_2017_filtered = lpr_2017_country.rename(columns={"Region and country of birth": "Country"})
## Remove columns that are not necessary 
lpr_2017_filtered = lpr_2017_filtered.drop(columns=['Non-CBSA', 'Other & Unknown'])
## drop missing data 
lpr_cols = lpr_2017_filtered.columns.drop('Country')
lpr_2017_final = lpr_2017_filtered.dropna(subset=lpr_cols, how='all').reset_index(drop=True)
## Check again 
print(lpr_2017_final)
## Saving files for the map - that will be need later
lpr_2017_final.to_csv('lpr_2017_by_country.csv', index=False)

## Part-3

## Filter out the Country and Total columns only 
lpr_2017_country_total = pd.read_csv('lpr_2017_by_country.csv')
...
lpr_countries = set(lpr_2017_country_total['Country'].str.strip())
...
lpr_2017_country_total['Country'] = lpr_2017_country_total['Country'].replace(to_rename_csv)
#
to_drop_csv = [
    'Soviet Union (former)',
...
countries_dropped = lpr_2017_country_total[~lpr_2017_country_total['Country'].isin(to_drop_csv)].reset_index(drop=True)
...
merged_iso_csv.to_csv('lpr_added_iso_cbsa_2017.csv', index=False)
...
merged_iso_csv_final.to_csv('lpr_2017_country_total_only.csv', index=False)
## Saved this for country specific maps 

#%%
## Top 20 countries of lpr flow in 2017

# Reading file
merged_lpr = pd.read_csv('lpr_2017_country_total_only.csv', thousands=',')
...
ax.set_title('Top 20 Countries by Total LPRs (2017)')
...
fig.tight_layout()
fig.savefig('top_20_lpr_2017.png', dpi=300)
# Saved the figure for top lpr by country of birth 

#%%
# Let's focus on the data with csba areas
cbsa_2017 = pd.read_csv('lpr_added_iso_cbsa_2017.csv', thousands=',')
cbsa_2017 = cbsa_2017.drop(columns=['Total', 'NAME_LONG', 'ISO_A3'])
cbsa_transposed = cbsa_2017.set_index('Country').T
...
merged_join_key.to_csv('merged_join_key_2017_for_QGIS.csv', index=False)

# Saved the file 
## Join in QGIS to show it on the map and 2017 analysis complete
###### COMPLETE for 2017
